...
function generateActivityVisualization(
  ...
  // Render UTC calendar dates consistently even when the host uses another timezone.
  const displayUTC = date =>
    new Date(
      date.getUTCFullYear(),
      date.getUTCMonth(),
      date.getUTCDate(),
  ...
  commitDateList = commitDateList.map(displayUTC);
  startDate = displayUTC(startDate);
  endDate = displayUTC(endDate);
  ...
  // Add a title
  result.push(chalk.bold.green("Generated activity preview:"));
  result.push("");
  ...
  result.push(`• Distribution: ${distribution}`);
